...
export default function FranchiseCard({ franchise, onClick }) {
  ...
  return (
    <button
      type="button"
      onClick={() => onClick?.(f)}
      className="w-full text-left rounded-md border border-slate-200 bg-emerald-50 hover:bg-emerald-100 shadow-sm hover:shadow-md transition focus:outline-none focus:ring-2 focus:ring-emerald-500"
    >
      {/* header row */}
      ...
      <div className="px-3 pb-3 mt-1 space-y-1 text-sm">
        ...
        <div className="flex items-center justify-between gap-2 text-slate-700">
          ...
          <span className="text-slate-600 text-xs">
            Area:{" "}
            <span className="text-emerald-700 font-medium">{f.area}</span>
          </span>
        </div>
      </div>
